...
for testIt in range(nTests):
    for sampleIt in range(nSamples):
        devFromSample = tstimg[testIt] - img[sampleIt]
...
answerPlt = np.reshape(tstimg[6],(28,28))
solutionPlt = np.reshape(img[int(match[6])],(28,28))

plt.suptitle('Correctly classified number')
